...
英辞郎・和英辞郎ともに、1物理行を1エントリとして扱う。CRLF を除去した後、見出し語の先頭に構造マーカー `■`（U+25A0）が1個あり、ASCII文字列 ` : ` がちょうど1個存在し、構造マーカーを除いた見出し語と本文がどちらも空でない行を正常とする。

```text
■headword : body
```

先頭構造マーカーは元TXTのレコード構造であり、見出し語の一部ではない。Parser が先頭の1個だけを除去し、DBには保存しない。構造マーカーの欠落、区切りの欠落、複数の区切り、空の見出し語、空の本文は malformed line とする。malformed line はDBへ登録せず、入力行番号と理由を警告として報告して変換を継続する。今回確認した和英辞郎の ` : ` が2個存在する行も、この規則によりskipする。
...
- `id`
  - 1始まりの元TXT物理行番号
- `headword`
  - 先頭構造マーカーを除き、それ以外の元表記を保持した表示用見出し語
- `headword_norm`
  - 検索用に正規化した見出し語
- `body`
  - 語義・説明
...
表示用 `headword` は、Parser が元データ先頭の `■`（U+25A0）構造マーカーを除去した後の表記を保持し、それ以外の正規化は行わない。構造マーカーは元TXTの構文であり、DB、アプリケーション表示、queryの一部として扱わない。Builder で見出し語から `headword_norm` を生成する処理と、検索アプリで query を正規化する処理には、辞書種別ごとに同一の正規化関数を使用する。検索文字数は正規化後の query に対して数える。
...
既定形式。各エントリを見出し語、本文、空行の順で出力する。DBに保存された、構造マーカーを含まない表示用 `headword` と `body` を改変しない。
...
fixture には先頭構造マーカー、CP932 固有文字、CRLF、不正な byte sequence を含む人工データを用意し、正常な decode、構造マーカーの除去、異常位置の報告を検証する。

構造マーカーの欠落、区切りの欠落、複数の区切り、空の見出し語、空の本文を含む人工データについて、該当行だけがskipされ、行番号・理由・集計値が正しく報告されることを検証する。重複見出し語はすべて保持されることも検証する。
