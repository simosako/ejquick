...
.PHONY: build build-gui package-gui-linux verify-gui-package test test-gui test-race test-desktop vet vet-gui fmt fmt-check tidy-check bench bench-smoke clean release-check
...
package-gui-linux:
	...
	QT_LICENSES_DIR="$(QT_LICENSES_DIR)" QT_ATTRIBUTIONS_FILE="$(QT_ATTRIBUTIONS_FILE)" \
	FCITX5_LICENSES_DIR="$(FCITX5_LICENSES_DIR)" FCITX5_COPYRIGHT_FILE="$(FCITX5_COPYRIGHT_FILE)" \
	ICU_LICENSE_FILE="$(ICU_LICENSE_FILE)" ICU_VERSION="$(ICU_VERSION)" \
	QTBASE_SOURCE_REV="$(QTBASE_SOURCE_REV)" QTWAYLAND_SOURCE_REV="$(QTWAYLAND_SOURCE_REV)" \
	FCITX5_QT_SOURCE_REV="$(FCITX5_QT_SOURCE_REV)" ICU_SOURCE_REV="$(ICU_SOURCE_REV)" \
		packaging/linux/package-gui-linux.sh

verify-gui-package:
	test -n "$(GUI_PACKAGE)" || { echo "GUI_PACKAGE is required" >&2; exit 1; }
	packaging/linux/verify-gui-package.sh "$(GUI_PACKAGE)" "$(VERSION)"
...
test-desktop:
	packaging/linux/test-desktop-scripts.sh
